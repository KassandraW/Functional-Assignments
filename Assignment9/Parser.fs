module Interpreter.Parser
    ...
    *)
    ...
    let CondParser = BTermParse .>*> pchar '?' .>*>. CondParse .>*> pchar ':' .>*>. CondParse |>> (fun((b1,aex1),aex2) -> Cond(b1,aex1,aex2))  <?> "conditional"
    ...
    //Level 4
    let NegParse = unop (pchar '-') AtomParse |>> (fun x -> Mul(Num -1, x) ) <?> "Negative"
    ...
    do aref := choice [NegParse; ParParse; BrackParse; ReadParse; RandomParse; NParse;  VParse]
